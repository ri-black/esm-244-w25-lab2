...
dep_var <- penguins_clean$mass
```


```{r}
# Model 1
f1   <-  mass~species+island+bill_l+bill_d+flip_l+sex
...
```{r}
# Model 2
f2   <-  mass~species+bill_l+bill_d+flip_l+sex
...
```{r}
# Model 3
f3   <-  mass~species+bill_d+flip_l+sex
...
```{r}
# Run AIC to justify model seleciton

AIC(mdl1,mdl2,mdl3)

# choose the LOWEST AIC value - this values represents the best fit model.
# in this case... model 2 has the smallest AIC
...
- split data into test and training
  - random sampling
- what metric to determine model fit?
  - Root Mean Squared Error (RMSE)
  - Let's make a funtion for RMSE

- for loop 
  - apply the model to each training set
  - make a prediction on the test set with fitted training model
- close loop

- summarize out RMSE (which model on average was best)
...
x <- rnorm(1000)     # random numbers of a given amount of times
y <- rnorm(1000)
...
calc_rmse_2<-function(x,y){      # same function just less easy to read
...
# choose model 1 in this case with the lowest rmse
```
...
### initialize a blank vector (storage vector)
rmse_vec<-vector(mode='numeric', length=folds)  #Why? we make this empty vector the appropriate size as a place to store output. If you don't know the length that the vector should be, use a 'While loop"
...
for( i in 1:folds){
  ...
  mutate(kfold_model1 = predict(kfold_training_lm1,kfold_test_df))
  
  # Summarize/calculate the rmse for that model
  kfold_rmse<-kfold_predict_test |> 
  summarize(rmse_md1=calc_rmse(mass,kfold_model1))
...
- for both AIC and RMSE scores suggest that model 2 is the best fit to predict penguin mass.
